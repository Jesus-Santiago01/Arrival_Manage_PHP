...
<footer></footer>
    ...
    Descargando</button>
    <button class="Botones" onclick="location.href='consultas.php'">
    Consultas</button>
        <br><br>
    ...
    <table class="table-rwd">
		...
		<?php foreach ($link->query("SELECT id,operador,placas,unidad,estatus,cortina,fecha,hora,tipo,documento FROM seller WHERE documento='' AND estatus='aceptado'") as $row){ // aca puedes hacer la consulta e iterarla con each.      ?>
  ...
  <div id="cerrar4">
    ...
    <?php foreach ($link->query("SELECT id,operador,placas,cortina,estatus,tipo,recibidor from seller WHERE estatus='descargando' AND cortina>0") as $row){ // aca puedes hacer la consulta e iterarla con each.      ?>
      <tr>
...
<thead>
  <tr> 
    ...
    <th>TIPO</th>
    <th>LLEGADA</th>
    <th>TIEMPO EN CORTINA</th>
    <th>DOCUMENTO</th>
    
  </tr>
</thead>
<?php foreach ($link->query("SELECT id,operador,placas,cortina,estatus,documento,tipo,cortina_hora,hora from seller WHERE estatus in ('aceptado','asignado') AND cortina>0") as $row){ // aca puedes hacer la consulta e iterarla con each.      ?>
  <tr>
  ...
  <div class="ventana2" id="ventana2">
    ...
    <?php foreach ($link->query("SELECT * from seller WHERE estatus='aceptado' AND cortina=0") as $row){ // aca puedes hacer la consulta e iterarla con each.      ?>
      <tr>
