...
Transfer</button>
<button class="Botones" onclick="location.href='recibocl.php'">
Colecta</button>
<h1>Formula IB</h1>
...
<div class="limite">
    ...
    <?php foreach ($link->query("SELECT * from seller WHERE estatus='aceptado' AND cortina>0 AND documento!='' AND tipo='f1' ORDER BY cortina_hora") as $row){ // aca puedes hacer la consulta e iterarla con each.      ?>
      <tr>
        <td> <?php echo $row['id'] ?> </td>
        <td> <?php echo $row['fecha']?><?php echo"     "?><?php echo $row['hora'] ?></td>
        <td> <?php echo $row['operador'] ?> </td>
        <td> <?php echo $row['cortina'] ?> </td>
        <td> <?php 
